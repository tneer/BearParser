...
public class App {
    ...
    private static void trello(final String boardName) {
        ...
        JsonObject model = Json.createObjectBuilder().add("name", boardName).add("idOrganization", DEN_ORG_NAME)
                                        .build();

        Client client = ClientBuilder.newClient(new ClientConfig().register(JsonProcessingFeature.class).property(
                                        JsonGenerator.PRETTY_PRINTING, true));
        client.register(LoggingFilter.class);

        WebTarget target = client.target(buildUrl("boards"));

        Response postResponse = target.request(MediaType.APPLICATION_JSON).post(
                                        Entity.entity(model.toString(), MediaType.APPLICATION_JSON));

        JsonObject responseJson = postResponse.readEntity(JsonObject.class);
        System.out.println(responseJson.getString("id"));
    }

    private static String buildUrl(final String typeName) {
        StringBuilder sb = new StringBuilder();
        sb.append("https://api.trello.com/1/").append(typeName + "?").append("key=" + TRELLO_KEY)
                                        .append("&token=" + TRELLO_TOKEN);
